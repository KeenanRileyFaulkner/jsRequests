...
const changeHelloButtonBack = () => {
    ...
    sayHelloButton.style.borderWidth = "thin";
    sayHelloButton.style.borderRadius = "2px"
}

sayHelloButton.addEventListener("mouseout", () => {
    changeHelloButtonBack();
})
...
const repeatMyParam = () => {
    //YOUR CODE HERE
    axios.get('http://localhost:3000/repeat/keenanfaulkner').then(res => {
...
// CODE HERE
const attachQuery = () => {
    axios.get('http://localhost:3000/query-test/?name=keenanfaulkner&programmer=yes').then(res => {
        console.log(res.data);
    })
}
